...
//#pragma omp parallel for
        for (size_t i = 0; i < n; i++){
            if (LQ_type == 2){
                ...
                for (size_t temp = 0; temp <dimension; temp ++){std::cout << subcentroid[temp] << " ";}std::cout << std::endl;
                faiss::fvec_madd(dimension, x + i * dimension, -1.0, subcentroid.data(), residuals + i * dimension);
                for (size_t temp = 0; temp <dimension; temp ++){std::cout << residuals[i * dimension + temp] << " ";}std::cout << std::endl;
            }
            else{
