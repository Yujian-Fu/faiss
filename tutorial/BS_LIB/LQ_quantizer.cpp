...
namespace bslib{
    ...
    LQ_quantizer::LQ_quantizer(size_t dimension, size_t nc_upper, size_t nc_per_group, const float * upper_centroids,
                               const idx_t * upper_centroid_ids, const float * upper_centroid_dists, bool use_all_HNSW):
...
#pragma omp parallel for
        ...
        if (use_all_HNSW){
            ...
            std::cout << "Finish construct the all_HNSW in LQ payer" << std::endl;
        }
    }
     ...
     * 
     **/
    void LQ_quantizer::compute_final_centroid(idx_t group_id, idx_t inner_group_id, float * final_centroid){
            ...
            faiss::fvec_madd(dimension, centroid, alphas[group_id], centroid_vector.data(), final_centroid);
    }
...
#pragma omp parallel for
     ...
     * 
     **/
    void LQ_quantizer::search_in_group(const float * query, idx_t * upper_result_labels, const float * upper_result_dists, const size_t upper_search_space, const idx_t group_id, float * result_dists, idx_t * result_labels){        
        ...
        for (size_t j = 0; j < nc_per_group; j++){
            ...
            if (nn_index < upper_search_space){
                ...
                result_dists[j] = alpha*(alpha-1)*group_nn_dist + (1-alpha)*query_group_dist + alpha*query_nn_dist;
            }
            else{
                ...
                result_dists[j] = faiss::fvec_norm_L2sqr(query_sub_centroid_vector.data(), dimension);
            }
        }
