...
int main(int argc, char * argv[]){
   ...
   // 把 now 转换为字符串形式
    char* dt = ctime(&now);
    path_record += "parameter_tuning_IVFADC_" + std::to_string(M_PQ) +"_" + std::string(dt) + ".txt";
    if (build_times == 0){
    ...
    std::vector<std::vector<idx_t>> best_recall_index_2(num_recall);
    std::vector<std::vector<size_t>> best_recall_para_1(num_recall);
    std::vector<std::vector<size_t>> best_recall_para_2(num_recall);
    std::vector<std::vector<float>> best_recall_time(num_recall);
    for (size_t i = 0; i < num_recall; i++){
        best_recall_time[i].resize(1000, 100);
        best_recall_index_1[i].resize(1000);
        best_recall_index_2[i].resize(1000);

        best_recall_para_1[i].resize(1000);
        best_recall_para_2[i].resize(1000);
    }*/

    record_file << "This is the record for IVFADC with centroids " << centroid1 << " and " << centroid2 << std::endl;

    //for (size_t centroid1 = VQ_min_centroid_space; centroid1 <= VQ_max_centroid_space; centroid1 += step_size){
        //size_t LQ_max_centroid = centroid1 / 10; size_t LQ_min_centroid = centroid1 / 50; size_t LQ_step_size = LQ_min_centroid;

        //for (size_t centroid2 = LQ_min_centroid; centroid2 <= LQ_max_centroid; centroid2 += LQ_step_size){

            memory_recorder Mrecorder = memory_recorder();
            time_recorder Trecorder = time_recorder();
            //record_file << "The result for IVFADC: " + std::to_string(centroid1) + " " + std::to_string(centroid2) << std::endl;
...
#pragma omp parallel for
        ...
        for (size_t i = 0; i < num_recall; i++){
            ...
            size_t c1_start = centroid1 / 25;
            size_t c1_end = centroid1 / 2;
            ...
            while(search_space1 < c1_end){
                ...
                if (qps < best_recall_time[i][size_t(recall*1000)]){
                    ...
                    best_recall_index_1[i][size_t(recall*1000)] = centroid1;
                    best_recall_index_2[i][size_t(recall*1000)] = centroid2;

                    best_recall_para_1[i][size_t(recall*1000)] = search_space1;
                    best_recall_para_2[i][size_t(recall*1000)] = search_space2;
                }*/
            }
        ...
        /*
        if (tested_constructions > 0 && (tested_constructions % 100) == 0){
            record_file << std::endl;
            for (size_t i = 0; i < num_recall; i++){
                record_file << "The record for recall " << result_k[i] << std::endl;
                for (size_t j = 0; j < 1000; j++){
                    record_file << best_recall_index_1[i][j] << " " <<  best_recall_index_2[i][j] << " " << best_recall_time[i][j] << " " << best_recall_para_1[i][j] << " " << best_recall_para_2[i][j];
                }
                record_file << std::endl;
            }
        }
