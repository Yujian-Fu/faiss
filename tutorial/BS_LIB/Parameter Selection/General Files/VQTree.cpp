   ...
   // 把 now 转换为字符串形式
    ...
    //for (size_t centroid1 = min_centroid_space; centroid1 <= max_centroid_space; centroid1 += step_size){
        //for (size_t centroid2 = min_centroid_space; centroid2 <= max_centroid_space; centroid2 += step_size){
            ...
            //record_file << "The result for VQ Tree: " + std::to_string(centroid1) + " " + std::to_string(centroid2) << std::endl;
...
#pragma omp parallel for
        ...
        for (size_t i = 0; i < num_recall; i++){
            ...
            size_t c2_start = centroid2 / 25;
            size_t c2_end = centroid2;
            size_t search_step2 = centroid2 / 25;
