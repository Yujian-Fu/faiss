...
int main(int argc, char * argv[]){
    ...
    //for (size_t centroid1 = min_centroid_space; centroid1 <= max_centroid_space; centroid1 += step_size){
        //for (size_t cons_nbits = min_nbits; cons_nbits <= max_nbits; cons_nbits += step_nbits){
...
#pragma omp parallel for
        ...
        for (size_t i = 0; i < num_recall; i++){
            ...
            while(search_space1 < c1_end && search_space2 < c2_end){
                ...
                if (qps < best_recall_time[i][size_t(recall*1000)]){
                    ...
                    best_recall_index_2[i][size_t(recall*1000)] = cons_nbits;
        ...
        if (tested_constructions > 0 && (tested_constructions % 100) == 0){
            record_file << std::endl;
            record_file << "The performance record for tested construction = " << tested_constructions << std::endl;
            for (size_t i = 0; i < num_recall; i++){
