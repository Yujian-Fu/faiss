...
recall10_result = []
recall100_result = []
record_file = "./kmeans/record/kmeans_PQSun_Nov_29_13_35_12_2020.txt"
#get_newest_folder("./kmeans/record", "kmeans_PQSun")
...
with open(record_file , 'r') as f:
    ...
    for line in lines:
        ...
        if index_distance_line:
            Kmeans_distance_result.append(float(line.split(" ")[-2]))
            index_distance_line = False
        
        if pq_distance_line:
            pq_distance_result.append(float(line.split(" ")[-2]))
            pq_distance_line = False
        ...
        if "Base vector - centroid distance" in line:
            index_distance_line = True
        if "Compression Loss distance" in line :
            pq_distance_line = True
